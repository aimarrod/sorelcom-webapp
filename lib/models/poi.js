...
POI.create = function(author, geojson, done){
    var id = 'POI' + geojson.properties.name.split(' ').join('_');

    var oldTriples = util.format('sorelcom:%s sioc:creator_of ?list . ',                        author);
    var newTriples = util.format('sorelcom:%s rdf:type sorelcom:POI . ',                        id) +
        ...
        util.format('sorelcom:%sgeom ogc:asWKT "%s"^^ogc:wktLiteral . ',                        id, geo.toWKT(geojson.geometry)) +
...
POI.list = function(done){
    var select = '?id rdf:type sorelcom:POI . ?id sioc:name ?name .';
