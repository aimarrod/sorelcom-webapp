...
module.exports.analyseGeojson = function(geojson){
	...
	var properties = {
		distance: 0,
		totalAscendingSlope: 0,
		totalDescendingSlope: 0,
		isCircular: false
	};

	/** Initial Altitude calculations */
	if(geometry.coordinates[0].length > 2){
		properties.maxAltitude = geometry.coordinates[0][2]; 
		properties.minAltitude = geometry.coordinates[0][2];
	} else {
		properties.maxAltitude = 0; 
		properties.minAltitude = 0;	
	}
	...
	for(var i = 1, l = geometry.coordinates.length; i < l; i++){
		...
		properties.distance += localDistance;

		/** ALtitude calculations */
		if(geometry.coordinates[i].length > 2){
			/* Max altitude */
			if(properties.maxAltitude < geometry.coordinates[i][2])
				properties.maxAltitude = geometry.coordinates[i][2];
			/* Min altitude */
			if(properties.minAltitude > geometry.coordinates[i][2])
				properties.minAltitude = geometry.coordinates[i][2];
			...
			if(geometry.coordinates[i-1].length > 2){
				...
				if(geometry.coordinates[i-1][2] > geometry.coordinates[i][2])
					properties.totalDescendingSlope += geometry.coordinates[i-1][2] - geometry.coordinates[i][2];
				/** Slope is ascending */
				else 
					properties.totalAscendingSlope += geometry.coordinates[i][2] - geometry.coordinates[i-1][2];	
			}
		}
	}
	properties.distance = Math.round(properties.distance);
...
module.exports.toBBox = function(bbox){
	var coords = bbox.split(',');
	wkt = util.format('POLYGON (%s %s, %s %s, %s %s, %s %s)', coords[0], coords[1], coords[0], coords[3], coords[2], coords[3], coords[2], coords[1]);
	console.log(wkt);
	return wkt;
};