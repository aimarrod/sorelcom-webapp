...
module.exports.create = function(args, done){
  ...
  if(args.difficulty !== undefined && args.difficulty !== null)
    params.triples += util.format('%s sorelcom:difficulty "%s"^^xsd:integer . ', uri, args.difficulty);
  if(args.distance)
    params.triples += util.format('%s sorelcom:distance "%s"^^xsd:long . ', uri, args.distance);
  if(args.maxAltitude)
    params.triples += util.format('%s sorelcom:maxAltitude "%s"^^xsd:float . ', uri, args.maxAltitude);
  if(args.altitude)
    params.triples += util.format('%s sorelcom:altitude "%s"^^xsd:float . ', uri, args.altitude);
  if(args.minAltitude)
    params.triples += util.format('%s sorelcom:minAltitude "%s"^^xsd:float . ', uri, args.minAltitude);
  if(args.averageAscendingSlope)
    params.triples += util.format('%s sorelcom:ascendingSlope "%s"^^xsd:float . ', uri, args.averageAscendingSlope);
  if(args.averageDescendingSlope)
    params.triples += util.format('%s sorelcom:descendingSlope "%s"^^xsd:float . ', uri, args.averageDescendingSlope);
  if(args.isCircular)
  ...
  /** POI Stuff */
  if(args.category)
    params.triples += util.format('%s sorelcom:category "%s". ', uri, args.category);
...
module.exports.showTrack = function(id, done){
	...
	params.bindings = '?id ?name ?content ?geometry ?author ?distance ?difficulty ?isCircular ?maxAltitude ?minAltitude ?totalAscendingSlope ?totalDescendingSlope';
  ...
  'OPTIONAL { ?id sorelcom:distance ?distance } . ' +
  'OPTIONAL { ?id sorelcom:difficulty ?difficulty } . ' +
  'OPTIONAL { ?id sorelcom:isCircular ?isCircular } . ' +
  'OPTIONAL { ?id sorelcom:maxAltitude ?maxAltitude } . ' +
  'OPTIONAL { ?id sorelcom:minAltitude ?minAltitude } . ' +
  'OPTIONAL { ?id sorelcom:ascendingSlope ?totalAscendingSlope } . ' +
  'OPTIONAL { ?id sorelcom:descendingSlope ?totalDescendingSlope } . ';
...
module.exports.showPOI = function(id, done){
  ...
  '?authorId sioc:name ?author . ' +
    /** Parameters data properties thay may not exist */
  'OPTIONAL { ?id sioc:content ?content } . ' +
  'OPTIONAL { ?id sorelcom:altitude ?maxAltitude } . ';
...
module.exports.within = function(args, done){
  ...
  params.bindings = '?id ?name ?geometry ?category ?difficulty';

  params.triples = util.format('?geom geo:sfIntersects [ a geo:Geometry; geo:asWKT "%s"^^geo:wktLiteral ] . ', args.geometry) +
  '?id geo:hasGeometry ?geom . ' +
  '?geom geo:asWKT ?geometry . ' +
  '?id sioc:name ?name . ' +
  'OPTIONAL { ?id sorelcom:category ?category } . ' +
  'OPTIONAL { ?id sorelcom:hasDifficulty ?difficulty } . ';

  if(args.type)
    params.triples += util.format('?id rdf:type %s', args.type);
