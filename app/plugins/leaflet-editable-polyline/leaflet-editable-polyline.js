L.Polyline.polylineEditor = L.Polyline.extend({
    _prepareMapIfNeeded: function() {
        ...
        if(this._map._editablePolylines != null && this._map._editablePolylines.length > 0) {
            return
        }
    ...
    _addMethods: function() {
        ...
        this._init = function(options, contexts) {
            ...
            var map = this._map;
            this._map.on("zoomend", this._mapevent);
            this._map.on("moveend", this._mapevent);
        ...
        this.removeEditable = function(layer, map){
            ...
            map.off('moveend', this._mapevent);
            map.off('zoomend', this._mapevent);
            map._editablePolylines.splice(map._editablePolylines.indexOf(this), 1);
        } 
